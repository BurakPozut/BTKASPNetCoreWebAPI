...
namespace Presentation.ActionFilters
{
    public class ValidateMediaTypeAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            {
                context.Result = new BadRequestObjectResult($"Accept header is miisng!");
                return;
            }

            var medieType = context.HttpContext.Request.Headers["Accept"].FirstOrDefault();

            if (!MediaTypeHeaderValue.TryParse(medieType, out MediaTypeHeaderValue? outMediaType))
            {
